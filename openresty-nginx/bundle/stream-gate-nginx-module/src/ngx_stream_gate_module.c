...
typedef enum {
    ...
    NGX_STREAM_ECHO_OPCODE_DISCARD_REQ,     /* for "echo_discard_request" */
} ngx_stream_gate_opcode_t;
...
typedef struct {
    ...
    ngx_stream_gate_opcode_t      opcode;
} ngx_stream_gate_cmd_t;


typedef struct {
    ngx_array_t      cmds;  /* of elements of type ngx_stream_gate_cmd_t */
...
} ngx_stream_gate_srv_conf_t;


typedef struct ngx_stream_gate_ctx_s  ngx_stream_gate_ctx_t;


typedef ngx_int_t (*ngx_stream_gate_filter_handler_pt)
        (ngx_stream_session_t *s, ngx_stream_gate_ctx_t *ctx);


struct ngx_stream_gate_ctx_s {
    ...
    ngx_stream_gate_filter_handler_pt        input_filter;
...
enum {
    ...
    NGX_STREAM_ECHO_DISCARD_BUFFER_SIZE   = 4096
};


static void ngx_stream_gate_handler(ngx_stream_session_t *s);
static void ngx_stream_gate_resume_execution(ngx_stream_session_t *s);
static ngx_int_t ngx_stream_gate_run_cmds(ngx_stream_session_t *s);
static ngx_int_t ngx_stream_gate_eval_args(ngx_array_t *raw_args,
    ngx_uint_t init, ngx_array_t *args, ngx_array_t *opts);
static ngx_int_t ngx_stream_gate_exec_echo(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static ngx_int_t ngx_stream_gate_exec_sleep(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static void ngx_stream_gate_cleanup(void *data);
static ngx_int_t ngx_stream_gate_exec_flush_wait(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static ngx_int_t ngx_stream_gate_exec_read_bytes(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static ngx_int_t ngx_stream_gate_read_bytes_filter(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx);
static ngx_int_t ngx_stream_gate_exec_read_line(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static ngx_int_t ngx_stream_gate_read_line_filter(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx);
static ngx_int_t ngx_stream_gate_do_read_request(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx);
static void ngx_stream_gate_read_request_handler(ngx_event_t *rev);
static ngx_int_t ngx_stream_gate_exec_echo_req(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static void ngx_stream_gate_adjust_buffer_in(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_chain_t *out);
static ngx_int_t
    ngx_stream_gate_exec_discard_req(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd);
static void ngx_stream_gate_discarded_request_handler(ngx_event_t *rev);
static ngx_int_t
    ngx_stream_gate_read_discarded_request(ngx_stream_session_t *s);
static void ngx_stream_gate_writer(ngx_event_t *wev);
static void ngx_stream_gate_sleep_event_handler(ngx_event_t *ev);
static void ngx_stream_gate_block_reading(ngx_event_t *ev);
static void ngx_stream_gate_finalize(ngx_stream_session_t *s, ngx_int_t rc);
static void ngx_stream_gate_set_lingering_close(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx);
static void ngx_stream_gate_lingering_close_handler(ngx_event_t *rev);
static void ngx_stream_gate_empty_handler(ngx_event_t *wev);
static ngx_stream_gate_ctx_t *
    ngx_stream_gate_create_ctx(ngx_stream_session_t *s);
static char *ngx_stream_gate_echo(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ngx_stream_gate_cmd_t *ngx_stream_gate_helper(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf, ngx_stream_gate_opcode_t opcode,
    ngx_array_t *args, ngx_array_t *opts);
static char *ngx_stream_gate_echo_duplicate(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ssize_t ngx_stream_gate_atosz(u_char *line, size_t n);
static char *ngx_stream_gate_echo_sleep(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_stream_gate_echo_flush_wait(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_stream_gate_echo_read_bytes(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_stream_gate_echo_read_line(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_stream_gate_echo_request_data(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_stream_gate_echo_discard_request(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static void *ngx_stream_gate_create_srv_conf(ngx_conf_t *cf);
static char *ngx_stream_gate_merge_srv_conf(ngx_conf_t *cf,
    void *parent, void *child);


static ngx_conf_enum_t  ngx_stream_gate_client_error_log_levels[] = {
...
static ngx_conf_enum_t  ngx_stream_gate_lingering_close[] = {
...
static ngx_command_t  ngx_stream_gate_commands[] = {

    { ngx_string("echo"),
      NGX_STREAM_SRV_CONF|NGX_CONF_ANY,
      ngx_stream_gate_echo,
    ...
    { ngx_string("echo_duplicate"),
      ...
      ngx_stream_gate_echo_duplicate,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("echo_flush_wait"),
      ...
      ngx_stream_gate_echo_flush_wait,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("echo_read_bytes"),
      ...
      ngx_stream_gate_echo_read_bytes,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("echo_read_line"),
      NGX_STREAM_SRV_CONF|NGX_CONF_NOARGS,
      ngx_stream_gate_echo_read_line,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("echo_request_data"),
      NGX_STREAM_SRV_CONF|NGX_CONF_NOARGS,
      ngx_stream_gate_echo_request_data,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("echo_discard_request"),
      NGX_STREAM_SRV_CONF|NGX_CONF_NOARGS,
      ngx_stream_gate_echo_discard_request,
      NGX_STREAM_SRV_CONF_OFFSET,
      0,
      NULL },
    ...
    { ngx_string("echo_send_timeout"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, send_timeout),
      NULL },
    ...
    { ngx_string("echo_read_timeout"),
      ...
      ngx_conf_set_msec_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, read_timeout),
      NULL },
    ...
    { ngx_string("echo_read_buffer_size"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, read_buffer_size),
      NULL },
    ...
    { ngx_string("echo_client_error_log_level"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, log_level),
      &ngx_stream_gate_client_error_log_levels },

    { ngx_string("echo_lingering_close"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_enum_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, lingering_close),
      &ngx_stream_gate_lingering_close },

    { ngx_string("echo_lingering_time"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_msec_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, lingering_time),
      NULL },
    ...
    { ngx_string("echo_lingering_timeout"),
      ...
      ngx_conf_set_msec_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_gate_srv_conf_t, lingering_timeout),
      NULL },
...
static ngx_stream_module_t  ngx_stream_gate_module_ctx = {
    ...
    ngx_stream_gate_create_srv_conf,       /* create server configuration */
    ngx_stream_gate_merge_srv_conf         /* merge server configuration */
};


ngx_module_t  ngx_stream_gate_module = {
    NGX_MODULE_V1,
    &ngx_stream_gate_module_ctx,           /* module context */
    ngx_stream_gate_commands,              /* module directives */
...
static void
ngx_stream_gate_handler(ngx_stream_session_t *s)
{
    ngx_connection_t            *c;
    ngx_stream_gate_ctx_t       *ctx;
    ngx_stream_gate_srv_conf_t  *escf;

    escf = ngx_stream_get_module_srv_conf(s, ngx_stream_gate_module);
    if (escf->cmds.nelts == 0) {
        /* cannot really happen */
        ngx_stream_gate_finalize(s, NGX_DECLINED);
        return;
    }

    c = s->connection;

    c->write->handler = ngx_stream_gate_writer;
    c->read->handler = ngx_stream_gate_block_reading;

    ctx = ngx_stream_gate_create_ctx(s);
    if (ctx == NULL) {
        ngx_stream_gate_finalize(s, NGX_ERROR);
        return;
    }

    ngx_stream_set_ctx(s, ctx, ngx_stream_gate_module);

    ngx_stream_gate_resume_execution(s);
}


static void
ngx_stream_gate_resume_execution(ngx_stream_session_t *s)
{
    ngx_int_t       rc;

    rc = ngx_stream_gate_run_cmds(s);

    dd("run cmds returned %d", (int) rc);

    ngx_stream_gate_finalize(s, rc);
}


static ngx_int_t
ngx_stream_gate_run_cmds(ngx_stream_session_t *s)
{
    ngx_int_t                        rc;
    ngx_buf_t                       *b;
    ngx_connection_t                *c;
    ngx_stream_gate_cmd_t           *cmd;
    ngx_stream_gate_ctx_t           *ctx;
    ngx_stream_gate_srv_conf_t      *escf;
    ...
    ctx = ngx_stream_get_module_ctx(s, ngx_stream_gate_module);
    if (ctx == NULL) {
        return NGX_ERROR;
    }
    ...
    while (ctx->cmd_index < escf->cmds.nelts) {
        ...
        case NGX_STREAM_ECHO_OPCODE_ECHO:
            rc = ngx_stream_gate_exec_echo(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_SLEEP:
            rc = ngx_stream_gate_exec_sleep(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_FLUSH_WAIT:
            rc = ngx_stream_gate_exec_flush_wait(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_READ_BYTES:
            rc = ngx_stream_gate_exec_read_bytes(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_READ_LINE:
            rc = ngx_stream_gate_exec_read_line(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_ECHO_REQ:
            rc = ngx_stream_gate_exec_echo_req(s, ctx, &cmd[ctx->cmd_index]);
            break;

        case NGX_STREAM_ECHO_OPCODE_DISCARD_REQ:
            rc = ngx_stream_gate_exec_discard_req(s, ctx, &cmd[ctx->cmd_index]);
            break;
...
static ngx_int_t
ngx_stream_gate_eval_args(ngx_array_t *raw_args, ngx_uint_t init,
    ngx_array_t *args, ngx_array_t *opts)
{
...
static ngx_int_t
ngx_stream_gate_exec_echo(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ngx_int_t            rc;
    ngx_chain_t         *out;
    ...
    out->buf->tag = (ngx_buf_tag_t) &ngx_stream_gate_module;
    ...
    ngx_chain_update_chains(c->pool, &ctx->free, &ctx->busy, &out,
                            (ngx_buf_tag_t) &ngx_stream_gate_module);
...
static ngx_int_t
ngx_stream_gate_exec_sleep(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ngx_connection_t    *c;
    ...
    if (ctx->cleanup == NULL) {
        ...
        ctx->cleanup->handler = ngx_stream_gate_cleanup;
...
static void
ngx_stream_gate_cleanup(void *data)
{
    ngx_stream_gate_ctx_t  *ctx = data;
...
static ngx_int_t
ngx_stream_gate_exec_flush_wait(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
...
static ngx_int_t
ngx_stream_gate_exec_read_bytes(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ngx_int_t                rc;
    ngx_connection_t        *c;
    ...
    ctx->input_filter = ngx_stream_gate_read_bytes_filter;

    rc = ngx_stream_gate_do_read_request(s, ctx);
    ...
    if (rc == NGX_DONE) {
        c->read->handler = ngx_stream_gate_read_request_handler;
        return NGX_DONE;
    }
...
static ngx_int_t
ngx_stream_gate_read_bytes_filter(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx)
{
    ssize_t              n;
    ngx_str_t           *req;
...
static ngx_int_t
ngx_stream_gate_exec_read_line(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ngx_int_t                rc;
    ngx_connection_t        *c;
    ...
    if (ctx->discard_req) {
        ngx_log_error(NGX_LOG_CRIT, c->log, 0,
                      "stream echo: echo_read_line not allowed after "
                      "echo_discard_request");
        return NGX_ERROR;
    }

    ctx->rest = cmd->data.size;
    ctx->input_filter = ngx_stream_gate_read_line_filter;
...
static ngx_int_t
ngx_stream_gate_read_line_filter(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx)
{
    u_char              *p, *q;
    size_t               len;
...
static ngx_int_t
ngx_stream_gate_do_read_request(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx)
{
    size_t               size;
    ssize_t              n;
...
static void
ngx_stream_gate_read_request_handler(ngx_event_t *rev)
{
    ngx_int_t                rc;
    ngx_connection_t        *c;
...
static ngx_int_t
ngx_stream_gate_exec_echo_req(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ngx_int_t                rc;
    ngx_chain_t             *out;
    ...
    if (ctx->discard_req || ctx->request.len == 0) {
        /* do nothing */
        return NGX_OK;
    }
...
#if 1
    if (rc == NGX_OK) {
        ngx_stream_gate_adjust_buffer_in(s, ctx, out);
    }
...
static void
ngx_stream_gate_adjust_buffer_in(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx, ngx_chain_t *out)
{
    size_t           len;
    ngx_buf_t       *b;
    ...
    ngx_stream_gate_ctx_t *ctx, ngx_stream_gate_cmd_t *cmd)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_STREAM, rev->log, 0,
                   "stream echo running discard-request");

    if (rev->timer_set) {
        ngx_del_timer(rev);
    }

    rc = ngx_stream_gate_read_discarded_request(s);
    ...
    rev->handler = ngx_stream_gate_discarded_request_handler;
...
static void
ngx_stream_gate_discarded_request_handler(ngx_event_t *rev)
{
    ngx_int_t                    rc;
    ngx_msec_t                   timer;
    ngx_connection_t            *c;
    ...
    if (rc == NGX_OK) {
        /* eof found */
        ngx_stream_gate_finalize(s, NGX_OK);
        return;
    }
    ...
    */
    ...
    for ( ;; ) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0,
                       "stream echo read %z bytes of discarded request data",
                       n);
    }
...
static void
ngx_stream_gate_writer(ngx_event_t *wev)
{
    ngx_int_t                    rc;
    ngx_chain_t                 *out;
    ngx_connection_t            *c;
    ...
    ngx_stream_gate_finalize(s, NGX_DONE);
}


static void
ngx_stream_gate_sleep_event_handler(ngx_event_t *ev)
{
    ngx_connection_t        *c;
    ngx_stream_session_t    *s;
...
static void
ngx_stream_gate_block_reading(ngx_event_t *ev)
{
    ngx_connection_t        *c;
    ngx_stream_session_t    *s;
...
static void
ngx_stream_gate_finalize(ngx_stream_session_t *s, ngx_int_t rc)
{
    ngx_connection_t            *c;
    ngx_stream_gate_ctx_t       *ctx;
    ngx_stream_gate_srv_conf_t  *escf;
    ...
    if (escf->lingering_close == NGX_STREAM_ECHO_LINGERING_ALWAYS
        || (escf->lingering_close == NGX_STREAM_ECHO_LINGERING_ON
            && ((ctx->buffer_in &&
                ...
                || c->read->ready)))
    {
        ngx_stream_gate_set_lingering_close(s, ctx);
        return;
    }
...
static void
ngx_stream_gate_set_lingering_close(ngx_stream_session_t *s,
    ngx_stream_gate_ctx_t *ctx)
{
    ngx_event_t                 *rev, *wev;
    ngx_connection_t            *c;
    ...
    rev = c->read;
    rev->handler = ngx_stream_gate_lingering_close_handler;

    ctx->lingering_time = ngx_time() + (time_t) (escf->lingering_time / 1000);
    ...
    wev = c->write;
    wev->handler = ngx_stream_gate_empty_handler;
    ...
    if (rev->ready) {
        ngx_stream_gate_lingering_close_handler(rev);
    }
}


static void
ngx_stream_gate_lingering_close_handler(ngx_event_t *rev)
{
    /* IOCP DOESN'T like buffer on stack, it need buffer in heap
...
static void
ngx_stream_gate_empty_handler(ngx_event_t *wev)
{
    ngx_log_debug0(NGX_LOG_DEBUG_STREAM, wev->log, 0,
...
ngx_stream_gate_create_ctx(ngx_stream_session_t *s)
{
    ...
    ctx = ngx_pcalloc(c->pool, sizeof(ngx_stream_gate_ctx_t));
    if (ctx == NULL) {
        return NULL;
    }
    ...
    ctx->sleep.handler   = ngx_stream_gate_sleep_event_handler;
    ctx->sleep.data      = s;
    ctx->sleep.log       = c->log;
...
static char *
ngx_stream_gate_echo(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    u_char          *p;
    size_t           size;
    ...
    ngx_stream_gate_cmd_t     *echo_cmd;

    echo_cmd = ngx_stream_gate_helper(cf, cmd, conf,
...
static ngx_stream_gate_cmd_t *
ngx_stream_gate_helper(ngx_conf_t *cf, ngx_command_t *cmd, void *conf,
    ngx_stream_gate_opcode_t opcode, ngx_array_t *args, ngx_array_t *opts)
{
    ngx_stream_gate_srv_conf_t  *escf = conf;
    ...
    if (escf->cmds.nelts == 0) {
        ...
        cscf->handler = ngx_stream_gate_handler;
    }
    ...
    if (args != NULL && opts != NULL) {
        ...
        if (ngx_stream_gate_eval_args(cf->args, 1, args, opts) != NGX_OK) {
            return NULL;
        }
    }
...
static char *
ngx_stream_gate_echo_duplicate(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    u_char          *p;
    size_t           size;
    ssize_t          n;
    ...
    n = ngx_stream_gate_atosz(arg[0].data, arg[0].len);
...
static ssize_t
ngx_stream_gate_atosz(u_char *line, size_t n)
{
    ssize_t  value;
...
static char *
ngx_stream_gate_echo_sleep(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t       *opt, *arg;
...
static char *
ngx_stream_gate_echo_flush_wait(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_stream_gate_cmd_t     *echo_cmd;
...
static char *
ngx_stream_gate_echo_read_bytes(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_stream_gate_srv_conf_t  *escf = conf;
...
static char *
ngx_stream_gate_echo_read_line(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_stream_gate_srv_conf_t  *escf = conf;
...
static char *
ngx_stream_gate_echo_request_data(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_stream_gate_cmd_t   *echo_cmd;
...
static char *
ngx_stream_gate_echo_discard_request(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_stream_gate_cmd_t   *echo_cmd;

    echo_cmd = ngx_stream_gate_helper(cf, cmd, conf,
                                      NGX_STREAM_ECHO_OPCODE_DISCARD_REQ,
                                      NULL, NULL);
    if (echo_cmd == NULL) {
        return NGX_CONF_ERROR;
    }
...
static void *
ngx_stream_gate_create_srv_conf(ngx_conf_t *cf)
{
    ngx_stream_gate_srv_conf_t  *conf;

    conf = ngx_palloc(cf->pool, sizeof(ngx_stream_gate_srv_conf_t));
    if (conf == NULL) {
        return NULL;
    }

    if (ngx_array_init(&conf->cmds, cf->pool, 1,
                       sizeof(ngx_stream_gate_cmd_t))
        == NGX_ERROR)
    {
        return NULL;
    }
...
static char *
ngx_stream_gate_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_stream_gate_srv_conf_t *prev = parent;
    ngx_stream_gate_srv_conf_t *conf = child;
