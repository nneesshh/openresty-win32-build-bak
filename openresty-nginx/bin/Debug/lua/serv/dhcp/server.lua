local cwd = (...):gsub('%.[^%.]+$', '') .. "."
local packet = require(cwd .. "packet")
...
_M.serve = function(callback)
    ...
    sock:settimeout(5000 * 1000)

    local data, err1 = sock:receive(2)
    if not data then
        return nil, "failed to read packet"
    end

    local packet, err2 = packet.new(data)
    if not packet then
        return nil, err2
    end
    ...
    local options, err3 = packet:options()
    if not options then
        return nil, err3
    end
