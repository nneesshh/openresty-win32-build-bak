...
local cwd = (...):gsub("%.[^%.]+$", "") .. "."
local xpcall = require(cwd .. "coxpcall").xpcall
...
-- defaults to 0.020
--  - `ringsize` (optional) number of slots in each ring, defaults to 90000 (0.5
...
-- @param wheel_now_time wheel now time
-- @return the timerwheel object
function _M.new(opts, wheel_now_time)
  assert(opts ~= _M, "new should not be called with colon ':' notation")

  opts = opts or EMPTY
  assert(type(opts) == "table", "expected options to be a table")

  local precision    = opts.precision or 0.020  -- in seconds, 20ms by default
  local precision_r  = 1/precision              -- slots num for per second step
  local ringsize     = opts.ringsize or 90000   -- #slots per ring, default 0.5 hour = 60 * 30 / 0.020
  local ringspan     = ringsize * precision     -- one ring span time
  ...
  local start     = wheel_now_time or now()
  local elapsed   = 0  -- 
  ...
  -- @param dt delta time since start time to now
  -- @return `true`
  function wheel:update(dt)
    ...
    elapsed = elapsed + dt
    new_position = math_floor(elapsed * precision_r) + 1
    ...
    while position < new_position do
      ...
      if position > ringsize then
        ...
        ring = rings[1] or EMPTY
        start = start + ringspan
        elapsed = elapsed - ringspan
        position = 1
      ...
      if slot then
        ...
        for i = 1, slot.n do
          ...
          id_list[id] = nil
          count = count - 1
          xpcall(cb, err_handler, arg, start+elapsed)
        end
  ...
  function wheel:set(expire_in, cb, arg, wheel_now_time)
    local wnow = wheel_now_time or now()
    local time_expire = wnow + expire_in
  ...
  -- end
  function wheel:peek(max_ahead, wheel_now_time)
    if count == 0 then
      return nil
    end
    local time_now = wheel_now_time or now()
